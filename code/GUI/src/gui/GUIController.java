...
<<<<<<< HEAD
    ...
    public static GUIController getInstance() {
        if (instance == null) {
            System.out
                    .println("Fehler in GUIController getInstance(). Application nicht gestartet. (instance == null).");
        }
        return instance;
    }
    ...
    private void reloadData() {
        ...
        Integer[] selectedCategoriesArray = allSelectedCategories
                .toArray(new Integer[allSelectedCategories.size()]);
        int i = 0;
        ...
        if (selectedCategoriesArray.length + selectedLocationsArray.length
                + selectedAccountsArray.length >= 1) {

            boolean dateSelected = selectedStartDate != null
                    && selectedEndDate != null;
            boolean success = true;
            try {
                dataByLocation = db.getSumOfData(selectedCategoriesArray,
                        selectedLocationsArray, selectedAccountsArray,
                        dateSelected);
                System.out.println("db.getSumOfData("
                        + selectedCategoriesArray.length + ","
                        + selectedLocationsArray.length + ","
                        + selectedAccountsArray.length + "," + dateSelected
                        + ");");
                dataByAccount = db.getAllData(selectedCategoriesArray,
                        selectedLocationsArray, selectedAccountsArray,
                        dateSelected);
            ...
            setInfo("Konnte keine Daten laden, bitte wählen Sie mindestens einen Filter.",
                    info);
            update(UpdateType.TWEET);
        }
    }
     ...
     * Earlier date will automatically be taken as start date. If one date is
     * null selected date range will be removed.
     * 
     * @param startDate
     *            of the date range
     * @param endDate
    ...
    public HashMap<String, Double> getDisplayValuePerCountry(
            TweetsAndRetweets tar, HashMap<String, Integer> retweetsPerLocation) {
        ...
        while (it.hasNext()) {
            ...
            double relativeValue = country.getCounter()
                    / (overallCounter * retweetsPerLocation.get(country
                            .getLocationCode()));
...
>>>>>>> 7fbc8f540a229c2f6b799eba667c0d90b35a7040
}
