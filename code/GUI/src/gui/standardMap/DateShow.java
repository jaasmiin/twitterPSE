...
public class DateShow extends Thread {
    ...
    private static final int DELAY_PER_DAY = 1000;
     ...
     *            instance of the GUIController
     * @param startDate
     *            start date of the 'show'
     * @param endDate
     *            end date of the 'show'
     */
    public DateShow(GUIController superController, LocalDate startDate,
            LocalDate endDate) {
     ...
     * 
     * @param date
     *            end date of the current intervall
    ...
    public void run() {
        ...
        while (!isInterrupted()
                && (currentDate.isBefore(endDate) || currentDate
                        .isEqual(endDate))) {
            ...
            calculatedData = superController.getDisplayValuePerCountry(
                    unCalcData, 1, startDate, currentDate);
            if (calculatedData != null) {
                map.update(calculatedData);
                map.redraw();
            }
            // show the displayed day
            superController.setInfo("Map currently shows the period from "
                    + startDate + " to " + currentDate, DELAY_PER_DAY);
