...
public class TimeSliderDialog extends JDialog {
    ...
    public void update(UpdateType type, LocalDate start, LocalDate end) {
        ...
        case TWEET:
            ...
            TweetsAndRetweets uneditedData = superController
                    .getDataByLocation();
            HashMap<String, Integer> forCalc = new HashMap<String, Integer>();
            for (mysql.result.Retweets r : uneditedData.getRetweets()) {
                Date startTest = new Date(start.getYear(),
                        start.getMonthValue(), start.getDayOfMonth());
                Date endTest = new Date(end.getYear(), end.getMonthValue(),
                        end.getDayOfMonth());
                System.out.println("retweet is after start " + r.getDate()
                        + " " + startTest.getDate() + " "
                        + r.getDate().after(startTest));
                // Check if Tweet/Retweet-Odjacts Date is in the needed interval
                if ((r.getDate().after(startTest) && r.getDate()
                        .before(endTest))
                        || r.getDate().equals(endTest)
                        || r.getDate().equals(startTest)) {
