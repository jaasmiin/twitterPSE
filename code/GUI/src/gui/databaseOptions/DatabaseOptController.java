 ...
 * 
 * 
 * @Override public void handle(Event event) { System.out.println("kontroller:"
 * + event.getSource());
 * 
 * //###################################################### //#PopUp Select
 * Account both versions
 * //########################################################
 * if(event.getSource().equals(txtAccountSearch)) { // update ListView of
 * accounts
 * 
 * String input = txtAccountSearch.getText(); listAccount.getItems().clear();
 * for(Account a : superController.getAccounts(input)) {
 * listAccount.getItems().add(a); } }
 * if(event.getSource().equals(bWeiterLocation)) { // save selected account,
 * close select-account-popUp
 * 
 * Account selectedAccount = listAccount.getSelectionModel().getSelectedItem();
 * if (selectedAccount == null) { lbInfo.setText("Kein Account ausgewählt");
 * return; } System.out.println(selectedAccount.getName()); dialogStage.close();
 * // popUp for InputDialog // Load the fxml file and create a new stage for the
 * popup
 * createPopUp("LocationSelect.fxml","Location auswählen",selectedAccount);
 * 
 * } if(event.getSource().equals(bWeiter)) { // save selected account, close
 ...
 * 
 * // popUp for InputDialog // Load the fxml file and create a new stage for the
 * popup createPopUp("CatSelect.fxml","Kategorie auswählen", selectedAccount);
 * 
 * }
 * 
 * 
 * //#################################################### //#PopUp select
 * category //####################################################
 * if(event.getSource().equals(txtCatSearch)) { // update TreeView of categories
 * String input = txtCatSearch.getText(); System.out.println(input);
 * 
 * } if(event.getSource().equals(trvCat)) { // add a category to list of
 * selected categories Category cat =
 * trvCat.getSelectionModel().getSelectedItem().getValue();
 * if(!listCat.getItems().contains(cat)) { // add a category only once
 * listCat.getItems().add(cat); }
 * 
 * } if(event.getSource().equals(bEntf)) { // remove a category from list of
 * selected categories int index
 * =listCat.getSelectionModel().getSelectedIndex(); System.out.println(index);
 * if (index >= 0) { listCat.getItems().remove(index);
 * listCat.setItems(listCat.getItems()); } }
 * if(event.getSource().equals(bFertig)) { // add selected Categories to
 * database System.out.println("bFertig xxxx"); for(Category cat :
 * listCat.getItems()) { superController.setCategory(account.getId(),
 * cat.getId()); } //System.out.println(DatabaseOptController.this);
 * dialogStage.close(); } if(event.getSource().equals(bZurueck)) { //close
 * select-category-popUp and open select-account-popUp again
 * dialogStage.close(); createPopUp("AccountSelect.fxml","Account auswählen",
 * null);
 * 
 * }
 * 
 * //#################################################### //#PopUp select
 * location //#################################################### /*
 * if(event.getSource().equals(bZurueckLocation)) { //close
 * select-location-popUp and open select-account-(location)-popUp again
 * dialogStage.close();
 * createPopUp("AccountSelectLocation.fxml","Account auswählen", null); }
 * if(event.getSource().equals(txtLocSearch)) { // update TreeView of locations
 * String input = txtLocSearch.getText(); System.out.println(input);
 * updateLocation(superController.getLocations(input)); }
 * if(event.getSource().equals(bFertigLocation)) { // add selected location to
 * database
 * 
 * TreeItem<Location> selectedItem =
 * trvLocation.getSelectionModel().getSelectedItem(); if (selectedItem == null)
 * { // no item selected; return; } if (account == null) {
 * System.out.println("an error occured!"); return; }
 * 
 * superController.setLocation(account.getId(),
 * selectedItem.getValue().getId()); dialogStage.close();
 * 
 * 
 * }
 * 
 ...
 * //####################################################
 * 
 * if(event.getSource().equals(bSearchAccount)) { // search for account matching
 * the query
 * 
 * String input = txtAccountAdd.getText(); if (input == null ||
 * input.equals("")) { return; } // clear old list
 * listAccountAdd.getItems().clear(); List<User> list =
 * TwitterAccess.getUser(input);
 * 
 * // fill found users/accounts in listView Iterator<User> it = list.iterator();
 * while(it.hasNext()) { Account account = new Account(0, it.next().getName(),
 * ""); listAccountAdd.getItems().add(account); }
 * 
 * } if(event.getSource().equals(bAccountAdd)) { // add account/user to database
 * listAccountAdd.getSelectionModel().getSelectedItem(); //Account account = new
 * Account() // TODO add superController.addAccount();
 * 
 * } if(event.getSource().equals(bAddAccountClose)) { dialogStage.close(); } }
 * 
 * }
 */
...
// popUp add account
/*
 * if (tabPane_Acc != null) { b_Acc_tab1_suchen.setOnMouseClicked(new
 * MyAccEventHandler()); b_Acc_tab1_hinzufuegen.setOnMouseClicked(new
 * MyAccEventHandler()); b_Acc_tab1_schliessen.setOnMouseClicked(new
 * MyAccEventHandler()); } //popUp select account category if(bWeiter != null) {
 * txtAccountSearch.setOnKeyReleased(new MyEventHandler());
 * listAccount.setOnMouseClicked(new MyEventHandler());
 * bWeiter.setOnMouseClicked(new MyEventHandler()); } //popUp add Category
 * if(txtCatSearch != null) { System.out.println("eingabe");
 * txtCatSearch.setOnKeyPressed(new MyEventHandler());
 * trvCat.setOnMouseClicked(new MyEventHandler()); bEntf.setOnMouseClicked(new
 * MyEventHandler()); bFertig.setOnMouseClicked(new MyEventHandler());
 * bZurueck.setOnMouseClicked(new MyEventHandler()); } //popUp select account
 * change location if(bWeiterLocation != null) {
 * bWeiterLocation.setOnMouseClicked(new MyEventHandler());
 * txtAccountSearch.setOnKeyPressed(new MyEventHandler());
 * listAccount.setOnMouseClicked(new MyEventHandler()); } //popUp change
 * location if(txtLocSearch != null) { txtLocSearch.setOnKeyPressed(new
 * MyEventHandler()); trvLocation.setOnMouseClicked(new MyEventHandler());
 * bFertigLocation.setOnMouseClicked(new MyEventHandler());
 * bZurueckLocation.setOnMouseClicked(new MyEventHandler()); } //popUp add
 * account if(txtAccountAdd != null) { bSearchAccount.setOnMouseClicked(new
 * MyEventHandler()); bAccountAdd.setOnMouseClicked(new MyEventHandler());
 * bAddAccountClose.setOnMouseClicked(new MyEventHandler()); } Tab tab = new
 * Tab();
 */
/*
 * 
 * @FXML private Button bFertig;
 * 
 * @FXML private Button bEntf;
 * 
 * @FXML private Button bZurueck;
 * 
 * @FXML private TreeView<Category> trvCat;
 * 
 * @FXML private TextField txtCatSearch;
 * 
 * @FXML private ListView<Category> listCat;
 * 
 * @FXML private TextField txtAccountSearch;
 * 
 * @FXML private Button bWeiter;
 * 
 * @FXML private ListView<Account> listAccount;
 * 
 * @FXML private Label lbInfo;
 * 
 * @FXML private TreeView<Location> trvLocation;
 * 
 * @FXML private TextField txtLocSearch;
 * 
 * @FXML private Button bWeiterLocation;
 * 
 * @FXML private Button bFertigLocation;
 * 
 * @FXML private Button bZurueckLocation;
 * 
 * @FXML private TabPane pane;
 * 
 * //AccountADD
 * 
 * @FXML private TextField txtAccountAdd;
 * 
 * @FXML private Button bSearchAccount;
 * 
 * @FXML private ListView<Account> listAccountAdd;
 * 
 * @FXML private Button bAccountAdd;
 * 
 * @FXML private Button bAddAccountClose;
 */
