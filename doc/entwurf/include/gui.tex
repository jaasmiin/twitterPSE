...
In dieser wird mittels eines von JavaFX bereitgestellten FXML-Loaders die Hauptverwaltungseinheit der GUI, bestehend aus ihrer Anzeige, der \emph{GuiView} und der zugehörigen Kontolle, dem \emph{GuiController} geladen und initialisiert.
Da diese aber schon die in das fxml-Dokument eingefügten GUI-Elemente enthalten müssen während des Ladevorgangs der Hauptverwaltungseinheit zuerst diese geladen werden.
Es können beliebig viele GUI-Elemente hinzugefügt werden. Hier wird das Element \emph{selectionOfQuery} exemplarisch herausgegriffen. 
Der FXML-Loader erstellt Anzeige und Kontrolle des GUI-Elements und startet dann die initialize-Methode des \emph{GuiControllers}.
Dieser verbindet sich darin mittels einer Instanz von \emph{DBIGui} mit der Datenbank und ruft dann die setParent-Methode des \emph{SelectionOfQueryController} auf.
Dieser erhält darin eine Referenz auf den \emph{GuiController}, setzt seine EventHandler und holt sich die zur Initialisierung benötigten Daten ab.
Der Vorgang ist beendet, sobald alle GUI-Elemente geladen und die Erstellung der Hauptverwaltungseinheit daraufhin abgeschlossen ist.
